import{a,c as n,b as e,o as i}from"./app-CYzvFbfO.js";const l={};function p(r,s){return i(),n("div",null,s[0]||(s[0]=[e(`<p>MyBatis-Flex 的读写分离功能是基于 【<a href="https://mybatis-flex.com/zh/core/multi-datasource.html" target="_blank" rel="noopener noreferrer">多数据源</a>】 功能来实现的。</p><p>读写分离的功能，要求当前环境必须是多个数据库（也可理解为多个数据源），其原理是： 让主数据库（master）处理事务性操作，比如：增、删、改（INSERT、DELETE、UPDATE），而从数据库（slave）处理查询（SELECT）操作。</p><h2 id="实现原理" tabindex="-1"><a class="header-anchor" href="#实现原理"><span>实现原理</span></a></h2><p>在 MyBatis 框架中，我们知道： 所有关于数据库的的操作都是通过 Mapper 来进行的，Mapper 里的一个方法，往往是和一个执行 SQL 一一对应。</p><p>因此，在 MyBatis-Flex 中，提供了一种基于 Mapper 方法的读写分离策略。</p><h2 id="数据源分片策略" tabindex="-1"><a class="header-anchor" href="#数据源分片策略"><span>数据源分片策略</span></a></h2><p>在 MyBatis-Flex 框架中，我们需要通过实现 <code>DataSourceShardingStrategy</code> 接口来自定义自己的数据源读写分离策略（分片策略）例如：</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#393a34;--shiki-dark:#dbd7caee;--shiki-light-bg:#ffffff;--shiki-dark-bg:#121212;"><pre class="shiki shiki-themes vitesse-light vitesse-dark vp-code"><code class="language-"><span class="line"><span>public class MyStrategy implements DataSourceShardingStrategy {</span></span>
